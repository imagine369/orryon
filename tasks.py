...
def create_data_sync_task() -> Task:
...
def create_net_worth_task() -> Task:
...
def create_budget_analysis_task(days: int = 30) -> Task:
...
def create_retirement_projection_task(params: dict | None = None) -> Task:
...
def create_cash_flow_forecast_task(months: int = 6) -> Task:
...
def create_schedule_task(user_query: str) -> Task:
...
def create_notes_task(user_query: str) -> Task:
...
def create_general_query_task(user_query: str) -> Task:
...
def create_net_worth_plus_retirement_tasks(user_query: str) -> list[Task]:
    """
...
def create_budget_plus_insights_tasks(user_query: str) -> list[Task]:
    """
    Two-task compound: budget analysis feeds into personalised insights.
    """
    budget_task = create_budget_analysis_task(days=30)
...
def route_query_to_tasks(user_query: str) -> list[Task]:
    """
    ...
    if any(kw in q for kw in [
        ...
        return [create_schedule_task(user_query)]
    ...
    if any(kw in q for kw in [
        ...
        return [create_notes_task(user_query)]
    ...
    if is_retirement and needs_net_worth:
        return create_net_worth_plus_retirement_tasks(user_query)
    if is_retirement:
        return [create_retirement_projection_task()]
    ...
    if is_budget and wants_insight:
        return create_budget_plus_insights_tasks(user_query)
    if is_budget:
        return [create_budget_analysis_task()]
    ...
    if any(kw in q for kw in [
        ...
        return [create_cash_flow_forecast_task()]
    ...
    # ── Default ───────────────────────────────────────────────────────────────
    return [create_general_query_task(user_query)]
