...
async def _run_startup() -> None:
    ...
    try:
        ...
        init_db()
        start_scheduler()
        logger.info("orryon backend started (AI: per-user Grok keys in Settings)")
        if XAI_API_KEY:
            logger.error(
                "XAI_API_KEY is set in the environment but is NOT used for chat. "
                "Remove it from Railway/Vercel so no one can spend on that key. "
                "Users must paste their own key in Settings → Grok."
            )
        logger.info("Request signing mode: %s", get_signing_mode())
        _log_email_config_status()

        # Mark ready before optional work so /api/ready and pytest are not
